...
\vspace{-25pt}
\begin{enumerate}
\item[2.] 平均標準化を行う関数は，スケーリングに使用する平均値や標準偏差も併せて返すようにし，新しいデータに対する平均標準化を行う関数の引数として使えるようにする．あとは，$\alpha $を変えながらひらすらデバッグをしていい感じに収束していくようなパターンを見つけていく．その結果，$\alpha = 0.3$の場合がいい感じであった．
\end{enumerate}
...
\subsection{クラスを用いた実装}

機械学習は，様々なモデルがあれど，処理の流れはデータを入れて学習し，未知のデータを入れて予測することである．機械学習は現実は様々なモデルを入れ替えながら，またパラメータを調整しながら何度も実行しそれらの結果を比較するので，変数の管理が非常に難しくなる．よって，機械学習のモデルやパラメータをクラスで組むことにより，インスタンスさえ区別しておけば，各モデルのパラメータ等は全てインスタンス変数として内部で保持してくれるため，管理が非常にしやすくなる．

\begin{qu}
問題\ref{qu_lr2_1}の問1について，クラス\texttt{MyLinearRegression}を作成し，\texttt{fit}メソッドでパラメータの推定，\texttt{predict}メソッドで予測するプログラムを作成し，結果が一致することを確認せよ．
\end{qu}
\begin{ans}
プログラム例は以下の通り．なお，$x_0=1$の列を追加する機能もクラスの内部で行うようにして，元データを加工せずに\texttt{fit}メソッドに放り込めるようにした．なお，\texttt{\_\_init\_\_}メソッドでは初期値を与えない場合には$\alpha=0.01$，繰り返し回数1500回が設定されるようにし，\texttt{fit}メソッドでは，パラメータの初期値を与えない場合は${\bm 0}$が設定されるようにした．
\begin{cod}[\texttt{lr14.py}]　
\lstinputlisting[backgroundcolor={\color[gray]{.95}}]{code/lr14.py}
\vspace{-10pt}
\begin{lstlisting}
y_pred=413220.18984796316
\end{lstlisting}
...
\begin{qu}
問題\ref{qu_lr2_1}の問2について，クラス\texttt{MyLinearRegression}(\texttt{fit}メソッドでパラメータの推定，\texttt{predict}メソッドで予測)，クラス\texttt{MyMeanNormalization}(\texttt{fit}メソッドで平均，標準偏差を算出しクラス内変数に保持，\texttt{transform}メソッドで特徴量を平均標準化したものを出力)を作成して線形回帰を実装し，結果が一致することを確認せよ．
\end{qu}
\begin{ans}
プログラム例は以下の通り．$x_0=1$を付加する部分は\texttt{MyLinearRegression}の内部で行なっているため，\ref{qu_lr2_1}の問2でやっているような，平均標準化をする上で1列目を避けて平均値などを計算するようなプログラムは書く必要がない．
\begin{cod}[\texttt{lr16.py}]　
\lstinputlisting[backgroundcolor={\color[gray]{.95}}]{code/lr16.py}
\vspace{-10pt}
\begin{lstlisting}
y_pred=411368.42281033465
\end{lstlisting}
...
\begin{center}
\framed
\includegraphics[width=8cm]{code/lrfig12.eps}
\vspace{-10pt}
\caption{\texttt{lrfig12.eps}}
\endframed
\end{center}
\end{figure}
\end{cod}
...
\lstinputlisting[backgroundcolor={\color[gray]{.95}}]{code/lr15.py}
\vspace{-10pt}
\begin{lstlisting}
...
scikit-learnは，機械学習のモデルのクラスだけではなく，特徴量スケーリングなどデータ前処理に関するクラスも多数備わっている．上で平均標準化のクラスをスクラッチ実装したが，scikit-learnにおいては\texttt{sklearn.preprocessing}の中にある\texttt{StandardScaler}クラスがそれに該当する．
\begin{gram}　
\begin{itemize}
...
\lstinputlisting[backgroundcolor={\color[gray]{.95}}]{code/lr17.py}
\vspace{-10pt}
\begin{lstlisting}
y_pred=411410.5372498779
\end{lstlisting}
